...
- **`chooseTurn(state, difficulty, evalFn?)`** runs negamax + alpha-beta over `generateTurns` and returns the chosen tap-sequence. `depth: 1` = greedy; `depth: 2` looks one reply ahead. The solo screen exposes three tiers via `DIFFICULTIES` (`easy` = depth 1 + 35% blunder chance, `medium` = clean depth 1, `hard` = depth 2); depth 3 is too slow interactively. `evalFn` lets an alternate evaluation (e.g. a future learned one) be swapped/A-B'd in.
